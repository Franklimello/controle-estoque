  ...
  return (
    <div
      ...
      className={`bg-white rounded-lg shadow-md p-6 cursor-pointer transition transform hover:scale-105 hover:shadow-lg ${
        hasWarning ? "border-l-4 border-red-500" : ""
      }`}
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-3">
          <div className={`p-3 rounded-full ${hasWarning ? "bg-red-100" : "bg-blue-100"}`}>
            <Package className={`w-6 h-6 ${hasWarning ? "text-red-600" : "text-blue-600"}`} />
          </div>
          <div>
            <h3 className="text-lg font-semibold text-gray-800">{item.nome}</h3>
            <p className="text-sm text-gray-500">Código: {item.codigo || "Sem código"}</p>
          </div>
        </div>
        <div className="flex items-center space-x-2">
          {isLowStock && (
            <div className="flex items-center space-x-1 text-red-500" title="Estoque baixo">
              <AlertTriangle className="w-5 h-5" />
            </div>
          )}
          {expiryInfo.isExpiring && !expiryInfo.isExpired && (
            <div className="flex items-center space-x-1 text-orange-500" title={`Vence em ${expiryInfo.daysUntilExpiry} dias`}>
              <Clock className="w-5 h-5" />
            </div>
          )}
          {expiryInfo.isExpired && (
            <div className="flex items-center space-x-1 text-red-600" title="Vencido">
              <AlertTriangle className="w-5 h-5" />
            </div>
          )}
        </div>
      ...
      <div className="mt-4 space-y-2">
        ...
        {item.validade && (
          <div className={`flex justify-between items-center pt-2 border-t ${
            expiryInfo.isExpired ? "bg-red-50 p-2 rounded" : 
            expiryInfo.isExpiring ? "bg-orange-50 p-2 rounded" : ""
          }`}>
            <span className="text-gray-600 font-medium">Validade:</span>
            <span
              className={`font-bold ${
                expiryInfo.isExpired ? "text-red-600" : 
                expiryInfo.isExpiring ? "text-orange-600" : "text-gray-800"
              }`}
            >
              {formatExpiryDate(item.validade)}
              {expiryInfo.isExpired && " (VENCIDO)"}
              {expiryInfo.isExpiring && !expiryInfo.isExpired && ` (${expiryInfo.daysUntilExpiry} dias)`}
            </span>
          </div>
        )}
        {item.updatedAt && (
